...
def test_renamed_key_migration():
    """The rename machinery, driven by an INJECTED pair rather than a real one.

    settings.RENAMED_KEYS is empty today -- the two entries it carried were retired along
    with the built-in temperature alerter. The mechanism still has to work, because the next
    rename will depend on it and an untested migration is one that silently does nothing, so
    the test supplies its own pair instead of deleting itself. `hub.low_load_threshold` is
    the stand-in target: any registry key would do, and the point is the row movement.
    """
    print("\n-- an override stored under a renamed key survives the rename --")
    original = settings.RENAMED_KEYS
    settings.RENAMED_KEYS = (("hub.old_low_load", "hub.low_load_threshold"),)
    try:
        db = fresh_db()
        # As a pre-rename hub stored a customised value. Raw SQL: nothing can write the old
        # key, which is exactly why the row would otherwise be unreachable -- _build() skips
        # keys that are not in the registry, so the operator's 95 would silently revert.
        with sqlite3.connect(db) as conn:
            conn.execute("INSERT INTO settings(key, value_json, updated_at, updated_by) "
                         "VALUES ('hub.old_low_load', '95', 1, 'op@x.com')")
        settings.init_settings_db(db)
        settings.invalidate()
        check("the customised value now resolves under the new key",
              settings.get(db, "hub.low_load_threshold") == 95)
        with sqlite3.connect(db) as conn:
            left = conn.execute("SELECT COUNT(*) FROM settings WHERE key='hub.old_low_load'"
                                ).fetchone()[0]
        ...
        with sqlite3.connect(db2) as conn:
            conn.execute("INSERT INTO settings(key, value_json, updated_at, updated_by) "
                         "VALUES ('hub.old_low_load', '95', 1, 'op@x.com')")
            conn.execute("INSERT INTO settings(key, value_json, updated_at, updated_by) "
                         "VALUES ('hub.low_load_threshold', '70', 2, 'op@x.com')")
        settings.init_settings_db(db2)
        settings.invalidate()
        check("a value already saved under the new key wins",
              settings.get(db2, "hub.low_load_threshold") == 70)
        with sqlite3.connect(db2) as conn:
            left = conn.execute("SELECT COUNT(*) FROM settings WHERE key='hub.old_low_load'"
                                ).fetchone()[0]
        check("...and the stale row is dropped, not left to collide", left == 0)
    finally:
        settings.RENAMED_KEYS = original


def test_retired_keys_are_gone():
    """The two temperature knobs are no longer settings at all.

    They moved into the rules engine, where a threshold sits next to the condition that uses
    it and can differ per machine. A leftover registry entry would put a field back on the
    Settings page that nothing reads -- the most confusing possible outcome for a knob whose
    whole point was that it controlled alerting.
    """
    print("\n-- the retired temperature knobs are not in the registry --")
    for key in ("hub.high_temp_threshold", "hub.high_temp_avg_window_seconds"):
        check(f"{key} is gone", key not in settings.BY_KEY)
        check(f"{key} cannot be written", _rejects(fresh_db(), {key: 60}))
...
def test_coercion():
    ...
    db = fresh_db()
    settings.set_many(db, {"hub.low_load_threshold": "95"})
    check('"95" coerces to int 95', settings.get(db, "hub.low_load_threshold") == 95)
    ...
    check("non-numeric int is rejected", _rejects(db, {"hub.low_load_threshold": "hot"}))
...
def test_range_validation():
    ...
    db = fresh_db()
    # hub.low_load_threshold is min=0 max=100.
    check("minimum is accepted", _accepts(db, {"hub.low_load_threshold": 0}))
    check("maximum is accepted", _accepts(db, {"hub.low_load_threshold": 100}))
    check("below minimum is rejected", _rejects(db, {"hub.low_load_threshold": -1}))
    check("above maximum is rejected", _rejects(db, {"hub.low_load_threshold": 101}))

    # hub.live_status_cache_seconds is min=60 max=86400.
    check("window minimum is accepted",
          _accepts(db, {"hub.live_status_cache_seconds": 60}))
    check("window maximum is accepted",
          _accepts(db, {"hub.live_status_cache_seconds": 86400}))
    check("window below minimum is rejected",
          _rejects(db, {"hub.live_status_cache_seconds": 59}))
    check("window above maximum is rejected",
          _rejects(db, {"hub.live_status_cache_seconds": 86401}))

    try:
        settings.set_many(db, {"hub.low_load_threshold": 500})
        check("out-of-range message names the field", False)
    except ValueError as e:
        check("out-of-range message names the field", "Low-load threshold" in str(e))
...
def test_set_many_is_all_or_nothing():
    ...
    try:
        settings.set_many(db, {
            ...
            "hub.low_load_threshold": 9999,        # out of range
            "hub.live_status_cache_seconds": 900,  # valid
        })
    ...
    check("the other valid field was NOT applied",
          settings.get(db, "hub.live_status_cache_seconds") == 600)
