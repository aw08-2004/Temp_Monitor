...
///
/// The buffer is the reason this class is not four lines, and it earns its keep here more than
/// on any other platform: a phone is off the network several times a day, in a lift, on a
/// plane, or simply out of range. A machine that loses its link keeps reading its own sensors;
/// on a connectivity failure a sensor-stripped copy of the payload is queued (bounded at 1000,
/// oldest dropped) and flushed oldest-first on the next success, so a reconnect backfills the
/// gap instead of leaving a hole in the history. Each queued copy carries the client_ts it was
/// BUILT with -- the hub backdates a reading to that timestamp (within
/// data.ingest_max_backdate_days), which is what makes the backfill land where it belongs on
/// the chart rather than all at once at reconnect.
///
/// **Sensors are stripped from a buffered copy on purpose.** A full sensor block is
/// kilobytes; a thousand of them is a device holding megabytes of history nobody asked for, on
/// a handset that may be offline precisely because something is wrong with it. The temperature
/// is what the history needs.
///
/// **The machine name is read at BUILD time, not at construction.** rename can change it while
/// the buffer holds readings, and a buffered payload must keep the name the device had when it
/// took the reading -- rewriting the queue on rename would move a week of history onto the new
/// machine, and reporting the old name for a queued item is what lets the hub's serial merge
/// put the two together properly.
///
...
/// before it has an identity, and that is what makes a device visible in the console within
...
public sealed class TelemetryReporter : IDisposable
{
        ...
        double temp, IReadOnlyList<SensorReading>? sensors, long? uptime, CancellationToken ct)
    {
        ...
        using (resp)
        {
            ...
            _log.LogDebug("Sent: {Temp} °C - hub responded {Status}", temp, status);
    ...
    /// <summary>How many readings are waiting to be backfilled. Read by the foreground
    /// notification, which is the only status display this agent has -- a phone has no
    /// journalctl, and "buffered 300 readings" on the lock screen is how someone standing next
    /// to the device learns it has been off the network since Tuesday.</summary>
    public int BufferedCount => _offline.Count;
