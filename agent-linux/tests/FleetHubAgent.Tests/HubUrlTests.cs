...
/// says yes to https://temp.arkeanos.net.attacker.net/x and to
/// https://temp.arkeanos.net@attacker.net/x, whose real hosts are attacker.net in both cases.
...
public class HubUrlTests
{
    ...
    public void Host_case_is_ignored() =>
        Assert.True(AgentConfig.IsHubUrl("https://TEMP.ARKEANOS.NET/api/report"));

    [Fact]
    public void The_default_port_is_filled_in() =>
        Assert.True(AgentConfig.IsHubUrl("https://temp.arkeanos.net:443/x"));

    [Theory]
    // The two that a StartsWith check gets wrong, which is the reason this function exists.
    [InlineData("https://temp.arkeanos.net.attacker.net/x")]
    [InlineData("https://temp.arkeanos.net@attacker.net/x")]
    // ...and the ordinary mismatches.
    [InlineData("http://temp.arkeanos.net/x")]        // scheme
    [InlineData("https://temp.arkeanos.net:8443/x")]  // port
