...
/// <summary>One chunk's fate: whether the hub took it, and whether it has heard enough.</summary>
public readonly record struct OutputPostResult(bool Ok, bool Truncated);

/// <summary>Where a running command's output goes. An interface rather than the client
/// itself so <see cref="OutputStreamer"/>'s ordering, retry and give-up behaviour can be
/// tested without a hub -- which is the whole of what is worth testing here, and none of it
/// is about HTTP.</summary>
...
/// instead of a spinner (roadmap #22). One instance per executing command.
///
/// Ported from the Windows agent's OutputStreamer with no behavioural changes, because the
/// contract it satisfies is the hub's and not the platform's. Its design notes carry over
/// whole, in rough order of how easy they are to break:
///
///  * ORDERING IS STRUCTURAL, not hoped for. A single flush loop owns _seq and never has two
///    POSTs in flight, so chunks cannot arrive out of order or race each other for a number.
///    Do not "optimise" this into parallel posts.
///  * A retry reuses the SAME seq. The hub keys on (command_id, seq) with INSERT OR IGNORE,
///    so a POST that actually landed before timing out is a free no-op. Allocating a fresh
///    seq on retry would duplicate the chunk instead.
///  * Add() is called from ProcessRunner's stdout/stderr callbacks and must never block them
...
///    stream forever. The full text still reaches the command result at completion, so a
///    dropped chunk costs live scrollback, not the record.
///
/// **The console shows this without any version gate**, which is why it is worth shipping on
/// an agent the console still reads as 0.1.0: fleet-terminal.js polls
/// /api/fleet/commands/&lt;id&gt;/output for any agent and prints what is there. What the
/// MIN_STREAMING_AGENT gate changes is the hint above the box and the interactive
/// affordances, neither of which this needs.
/// </summary>
public sealed class OutputStreamer : IAsyncDisposable
{
    ...
    ///
    /// The text is appended verbatim -- no trailing newline is added here, because a producer
    /// that delivers partial lines would have them corrupted by one. Line-oriented producers
    /// (ProcessRunner) append their own before calling this.</summary>
    public void Add(string text)
    {
        lock (_gate)
        {
            ...
            _buffer.Append(text);
            // Only nudge the pump when we are already at the size threshold; otherwise let
    ...
    /// status, so a result that lands ahead of the final chunk silently truncates the tail
    /// the operator sees.</summary>
    ...
    private async Task FlushOnceAsync(CancellationToken ct)
    {
        while (true)
        {
            ...
            lock (_gate)
            {
                ...
                // Split oversized buffers so the hub never has to reject one. Whole
    ...
    /// <summary>Post one chunk, retrying the same seq. Returns false when the caller should
    /// stop flushing (the hub truncated, or we gave up on this chunk).</summary>
